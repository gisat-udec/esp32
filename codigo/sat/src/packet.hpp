...
struct Packet {
    ...
    Packet(uint8_t type, uint8_t subtype, uint16_t length) :
        type(type), subtype(subtype), length(length) {
        data.reset(new uint8_t[length]());
    }
    Packet(uint8_t type, uint8_t subtype, uint16_t length, void *buf) :
        type(type), subtype(subtype), length(length) {
        data.reset(new uint8_t[length]());
    ...
    Packet(void *buf) {
        ...
        std::copy_n(_buf, header_len(), &type);
